...
const logger = createLogger('API:trends/category');

/**
 * API route to fetch MercadoLibre trends for a specific country and category
...
export async function GET(
  ...
  logger.info(`Fetching trends for ${country} in category ${category}`);
  ...
  try {
    ...
    const trendsResponse = await fetch(
      `https://api.mercadolibre.com/trends/${country}/${category}`,
      {
        headers: {
    ...
    logger.success(
      `Trends fetched successfully for ${country}/${category}: ${trends.length} trends`,
    ...
    logger.error(
      `Trends endpoint failed for ${country}/${category}`,
